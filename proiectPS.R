...
frepcomgen <- function(n, m) {
  ...
  # facem normalizarea fiecărei linii pentru a ne asigura că suma fiecărei linii este 1
  xycomp <- t(apply(xycomp, 1, function(row) row / sum(row)))
  
  # facem normalizarea globală pentru a ne asigura că suma totală a elementelor este 1
...
completare <- function(mtx, indice_linie, indice_coloana, tip_completare){
  ...
  suma_pi <- as.numeric(mtx[indice_linie,m]) #extragem valoarea probabilității marginale pe linia dată
  suma_qi <- as.numeric(mtx[n,indice_coloana]) #extragem valoarea probabilității marginale pe coloana dată
  #Daca tip_completare este 1 înseamnă că vom face completarea la nivel de linie
  if(tip_completare==1){
    for(j in 2:(m-1)){
      if(indice_coloana!=j){
  ...
  #Dacă tip_completare este 0 înseamnă că vom face completarea la nivel de coloană
  else{
    for(i in 1:(n-1)){
      if(indice_linie!=i){
        #calculăm valoarea lipsă prin scăderi repetate
...
#FUNCȚIA PRINCIPALĂ CARE IDENTIFICĂ VALORILE LIPSĂ(NaN) PE CARE LE PUTEM COMPLETA
fcomplrepcom <- function(mtx) {
  ...
  for (i in 1:(n-1)) {
    ...
    for (j in 2:(m-1)) {
      if (is.nan(as.numeric(mtx[i, j]))) {
        #daca gasim o valoare NaN incrementăm contorul și salvăm coloana pe care am găsit elementul 
        contor_linie  <- contor_linie + 1
        indice_col <- j
      }
    }
    #dacă am găsit un singur element lipsă pe linia i înseamnă că putem face completarea
    if(contor_linie == 1)
  ...
  for(j in 2:(m-1)){
    ...
    for(i in 1: (n-1)){
      if (is.nan(as.numeric(mtx[i, j]))) {
        #daca gasim o valoare NaN incrementăm contorul și salvăm linia pe care am găsit elementul 
        contor_coloana  <- contor_coloana + 1
        indice_lin <- i
      }
    }
    
    #dacă am găsit un singur element lipsă pe coloana j înseamnă că putem face completarea
    if(contor_coloana==1){
...
calcul_g.2 <- function (mtx){
  ...
  for(i in 1:(nrow(mtx)-1)){
    for(j in 2:(ncol(mtx)-1)){
      #dintr-o eroare de logistica nu putem accesa indicii valorilor lui Y deoarece este un rand care nu face efectiv parte din matrice
      #Oups...
...
#un punct(bulina) din interiorul reprezentarii vizuale, si difera ca dimensiune
#in functie de valoarea ei (dimensiunea unui punct 
#e direct proportionala cu valoarea probabilitatii).
...
#(Generarea repartitiei comune sub forma: "eticheta X", "eticheta Y", "eticheta Z", probabilitate)
...
test_fubini <- function(f, a, b, c, d) {
  if (!is.na(integrate(function(x) f(x, a), lower = a, upper = b)$value) && #daca integrala primei functii e finita
      !is.na(integrate(function(y) f(a, y), lower = c, upper = d)$value)) { #daca integrala functiei 2 e finita
